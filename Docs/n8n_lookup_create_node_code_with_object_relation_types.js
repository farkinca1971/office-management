 ...
 *            'identification-types', 'transaction-types', 'currencies', 
 *            'object-relation-types', 'audit-types', 'audit-actions',
 *            'document-types', 'note-types'
 *   Note: Hyphens are automatically converted to underscores
 * 
 * Request Body (required fields):
 * - code: Unique code for the lookup item (required)
 * 
 * Request Body (optional fields):
 * - is_active: Active status (default: true)
 * - text: Translation text for the current language
 * - language_id: Language ID for the translation
 * - object_type_id: For object-statuses, identification-types, and audit-actions - the object type this belongs to
 * - parent_object_type_id: For object-relation-types - the parent object type ID (optional)
 * - child_object_type_id: For object-relation-types - the child object type ID (optional)
 * - mirrored_type_id: For object-relation-types - the mirrored relation type ID (optional, can be null)
 * 
 * Behavior:
 ...
 * - For object-statuses, identification-types, and audit-actions, object_type_id is required
 * - For object-relation-types, parent_object_type_id, child_object_type_id, and mirrored_type_id are optional
 * 
...
const inputData = $('Create Lookup').first().json;
...
// Extract object_relation_types specific fields
const parentObjectTypeId = inputData.body?.parent_object_type_id;
const childObjectTypeId = inputData.body?.child_object_type_id;
const mirroredTypeId = inputData.body?.mirrored_type_id; // Can be null
...
// Special validation for object_statuses, identification_types, and audit_actions (all require object_type_id)
if ((lookupType === 'object_statuses' || lookupType === 'identification_types' || lookupType === 'audit_actions') && !objectTypeId) {
  return {
    success: false,
    error: {
      code: 'MISSING_OBJECT_TYPE_ID',
      message: `object_type_id is required for ${lookupType}`,
      details: {
...
// Validate object_relation_types specific fields if provided
if (lookupType === 'object_relation_types') {
  if (parentObjectTypeId !== undefined && parentObjectTypeId !== null) {
    const parsedParentObjectTypeId = parseInt(parentObjectTypeId);
    if (isNaN(parsedParentObjectTypeId)) {
      return {
        success: false,
        error: {
          code: 'INVALID_PARENT_OBJECT_TYPE_ID',
          message: 'parent_object_type_id must be a valid integer',
          details: {
            provided: parentObjectTypeId
          }
        }
      };
    }
  ...
  if (childObjectTypeId !== undefined && childObjectTypeId !== null) {
    const parsedChildObjectTypeId = parseInt(childObjectTypeId);
    if (isNaN(parsedChildObjectTypeId)) {
      return {
        success: false,
        error: {
          code: 'INVALID_CHILD_OBJECT_TYPE_ID',
          message: 'child_object_type_id must be a valid integer',
          details: {
            provided: childObjectTypeId
          }
        }
      };
    }
  ...
  if (mirroredTypeId !== undefined && mirroredTypeId !== null) {
    const parsedMirroredTypeId = parseInt(mirroredTypeId);
    if (isNaN(parsedMirroredTypeId)) {
      return {
        success: false,
        error: {
          code: 'INVALID_MIRRORED_TYPE_ID',
          message: 'mirrored_type_id must be a valid integer or null',
          details: {
            provided: mirroredTypeId
          }
        }
      };
    }
...
const escapedCode = createCode.replace(/'/g, "''");
...
const validationQueries = [
  {
    ...
    description: `Check if code '${escapedCode}' already exists in ${tableName} table`
  },
  {
    query: checkTranslationsTableQuery.trim(),
    type: 'validation_translation_check',
    description: `Check if code '${escapedCode}' already exists in translations table`
  }
];
...
if (lookupType === 'object_statuses' || lookupType === 'identification_types' || lookupType === 'audit_actions') {
  // object_statuses, identification_types, and audit_actions all require object_type_id
  const parsedObjectTypeId = parseInt(objectTypeId);
...
} else if (lookupType === 'object_relation_types') {
  // object_relation_types has optional parent_object_type_id, child_object_type_id, and mirrored_type_id
  const columns = ['code', 'is_active'];
  const values = [`'${escapedCode}'`, `${isActiveValue}`];
  
  // Add parent_object_type_id if provided
  if (parentObjectTypeId !== undefined && parentObjectTypeId !== null) {
    const parsedParentObjectTypeId = parseInt(parentObjectTypeId);
    columns.push('parent_object_type_id');
    values.push(`${parsedParentObjectTypeId}`);
  }
  
  // Add child_object_type_id if provided
  if (childObjectTypeId !== undefined && childObjectTypeId !== null) {
    const parsedChildObjectTypeId = parseInt(childObjectTypeId);
    columns.push('child_object_type_id');
    values.push(`${parsedChildObjectTypeId}`);
  }
  
  // Add mirrored_type_id if provided (can be null, but we only include it if explicitly provided)
  if (mirroredTypeId !== undefined) {
    if (mirroredTypeId === null) {
      columns.push('mirrored_type_id');
      values.push('NULL');
    } else {
      const parsedMirroredTypeId = parseInt(mirroredTypeId);
      columns.push('mirrored_type_id');
      values.push(`${parsedMirroredTypeId}`);
    }
  }
  
  lookupInsertQuery = `INSERT INTO ${tableName} (${columns.join(', ')}) VALUES (${values.join(', ')})`;
} else {
  // All other lookup tables (no object_type_id required)
  // This includes: languages, object_types, sexes, salutations, product_categories, countries,
  // address_types, address_area_types, contact_types, transaction_types, currencies,
  // audit_types, document_types, note_types
...
if (hasTranslation) {
  ...
  const escapedText = createText.replace(/'/g, "''");
...
allQueries.push({
  ...
  type: 'lookup_insert',
  description: `Insert ${tableName} record with code '${escapedCode}'`
});
...
return {
  json: {
    ...
    create_fields: {
      ...
      child_object_type_id: childObjectTypeId,
      mirrored_type_id: mirroredTypeId
    },
    ...
    metadata: {
      ...
      tables_requiring_object_type_id: ['object_statuses', 'identification_types', 'audit_actions'],
      object_relation_types_fields: {
        parent_object_type_id: 'Optional - Parent object type ID',
        child_object_type_id: 'Optional - Child object type ID',
        mirrored_type_id: 'Optional - Mirrored relation type ID (can be null)'
      }
    }
  }
};
