/**
 * n8n Code Node - Dynamic Lookup Table Query Handler (Updated with audit_actions, identification_types, and object_relation_types)
 *
 * This single node can handle all lookup/reference data queries including audit_actions, identification_types, and object_relation_types
 *
 * Setup:
 ...
 * - parent_object_type_id: Filter for object_relation_types - parent object type in the relationship (optional)
 * - child_object_type_id: Filter for object_relation_types - child object type in the relationship (optional)
...
const parentObjectTypeId = inputData.query?.parent_object_type_id || inputData.parent_object_type_id || inputData.body?.parent_object_type_id;
const childObjectTypeId = inputData.query?.child_object_type_id || inputData.child_object_type_id || inputData.body?.child_object_type_id;
...
ORDER BY aa.code;
  ...
  case 'object_relation_types':
    // Object Relation Types: supports filtering by parent_object_type_id and child_object_type_id
    // The table structure includes parent_object_type_id, child_object_type_id, and mirrored_type_id
    // This allows relationships like Person -> Person (family), Company -> Person (employer/worker), etc.
    const ortWhere = [];
    ortWhere.push('ort.is_active = 1');
    if (parentObjectTypeId) {
      ortWhere.push(`ort.parent_object_type_id = ${parseInt(parentObjectTypeId)}`);
    }
    if (childObjectTypeId) {
      ortWhere.push(`ort.child_object_type_id = ${parseInt(childObjectTypeId)}`);
    }
...
return {
  json: {
    ...
    parameters: {
      ...
      parent_object_type_id: parentObjectTypeId || null,
      child_object_type_id: childObjectTypeId || null,
      code: code || null,
