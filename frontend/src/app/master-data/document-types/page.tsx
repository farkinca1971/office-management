...
export default function DocumentTypesPage() {
  ...
  const loadData = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await lookupApi.getDocumentTypes(language);
      if (response.success) {
      ...
      setError(err?.error?.message || 'Failed to load document types');
    } finally {
      setIsLoading(false);
    }
  }, [language]);
  ...
  const handleCreate = async (item: { code: string; is_active?: boolean; text?: string; language_id?: number }) => {
    const response = await lookupApi.createDocumentType(item);
    if (response.success) {
      await loadData();
    } else {
  ...
  const handleUpdate = async (id: number, item: {
    ...
    const response = await lookupApi.updateDocumentType(id, updatePayload);
    if (response.success) {
      await loadData();
    } else {
  ...
  const handleDelete = async (id: number) => {
    const response = await lookupApi.deleteDocumentType(id);
    if (response.success) {
      await loadData();
    } else {
