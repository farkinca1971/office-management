/**
 * Identification Types Lookup Page
 */
...
export default function IdentificationTypesPage() {
  ...
  const loadData = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const [typesResponse, objectTypesResponse] = await Promise.all([
        lookupApi.getIdentificationTypes(language),
        lookupApi.getObjectTypes(language)
      ]);
      
      if (typesResponse.success) {
        // Create a new array reference to ensure React detects the change
        setData([...typesResponse.data]);
      } else {
        setError('Failed to load identification types');
      }
      ...
      setError(err?.error?.message || 'Failed to load identification types');
    } finally {
      setIsLoading(false);
    }
  }, [language]);
  ...
  const handleCreate = async (item: { code: string; is_active?: boolean; text?: string; language_id?: number; object_type_id?: number }) => {
    ...
    const response = await lookupApi.createIdentificationType(createPayload);
    if (response.success) {
      // Clear error state on success
      setError(null);
      await loadData();
    } else {
      const errorMessage = response.error?.message || 'Failed to create identification type';
  ...
  const handleUpdate = async (id: number, item: { 
    ...
    const response = await lookupApi.updateIdentificationType(id, updatePayload);
    if (response.success) {
      // Clear error state on success
      setError(null);
      await loadData();
    } else {
      const errorMessage = response.error?.message || 'Failed to update identification type';
      setError(errorMessage);
      throw new Error(errorMessage);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    const response = await lookupApi.deleteIdentificationType(id);
    if (response.success) {
      // Clear error state on success
      setError(null);
      await loadData();
    } else {
      const errorMessage = response.error?.message || 'Failed to delete identification type';
      setError(errorMessage);
      throw new Error(errorMessage);
    }
  };
  ...
  return (
    <LookupTable
      title={t('nav.identificationTypes')}
      data={data}
      isLoading={isLoading}
      error={error}
      onLoad={loadData}
