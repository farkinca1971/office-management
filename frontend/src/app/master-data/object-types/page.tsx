...
export default function ObjectTypesPage() {
  ...
  const loadData = React.useCallback(async () => {
    ...
    try {
      ...
      setError(err?.error?.message || 'Failed to load object types');
    } finally {
      setIsLoading(false);
    }
  }, [language]);
  ...
  const handleCreate = async (item: { code: string; is_active?: boolean; text?: string; language_id?: number }) => {
    const response = await lookupApi.createObjectType(item);
    if (response.success) {
      await loadData();
    } else {
  ...
  const handleUpdate = async (id: number, item: { 
    ...
    const response = await lookupApi.updateObjectType(id, updatePayload);
    if (response.success) {
      await loadData();
    } else {
      throw new Error('Failed to update object type');
    }
  };

  const handleDelete = async (id: number) => {
    const response = await lookupApi.deleteObjectType(id);
    if (response.success) {
      await loadData();
    } else {
      throw new Error('Failed to delete object type');
    }
  };
